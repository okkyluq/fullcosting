...
<section class="content">
    <div class="row">
		<div class="col-md-6 text-left" style="margin-bottom: 20px;">
            <a href="{{url('bahan/create')}}" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah Data</a>
		</div>
	</div>
				...
				<div class="box-body">
					@if (!empty($bahan))
						<table class="table">
							...
							<tbody>
								@php $no=1 @endphp
								@foreach ($bahan as $isi)
								<tr>
									<td>{{$no++}}</td>
									<td>{{$isi->kode_bahanbaku}}</td>
									<td>{{ $isi->nama }}</td>
									<td>{{$isi->satuan}} </td>
									<td>{{date_format($isi->created_at, 'Y-m-d')}}</td>
									<td class="text-center">
										<div class="box-button" style="display: inline-block;">
											<a href="{{url('bahan/'.$isi->id.'/edit')}}" class="btn btn-xs btn-success"><i class="fa fa-edit"></i></a>
										</div>
										<div class="box-button" style="display: inline-block;">
											<form action="{{url('bahan/'.$isi->id)}}" method="POST">
